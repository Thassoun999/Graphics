...
void main()
{
 ...
 // Compute terms in the illumination equation
    vec4 ambient = light_ambient * ini_material_ambient;
    float d = max( dot(L, N), 0.0 );
    vec4  diffuse = d * light_diffuse * ini_material_diffuse;
    float s = pow( max(dot(N, H), 0.0), Shininess );
    vec4  specular = s * light_specular * ini_material_specular;
    if( dot(L, N) < 0.0 ) {
   ...
   vec3 spot_l = -L;
   vec3 spot_l_f= normalize( spotDir.xyz-LightPosition.xyz );

   float spot_att=pow(dot(spot_l,spot_l_f), spotExp);

   vec4 color_add_spot=spot_att*color_add_point;

   if ( dot(spot_l,spot_l_f) < spotAng)
   ...
   if (shadeFlag == 0) {
	...
	if(draw_shadow_lat==1)
	{
		if(eye_space_flag == 1)
		{
			if(upright_lat_flag==1)
	  		{
				latticeCoordinate[0] = (pos[0] + 1) * 0.5;
				latticeCoordinate[1] = (pos[1] + 1) * 0.5;
			}
			else
	  		{
				latticeCoordinate[0] = (pos[0] + pos[1] + pos[2]) * 0.3; 
				latticeCoordinate[1] = (pos[0] - pos[1] + pos[2]) * 0.3;
			} 
		}
		else if(object_space_flag == 1)
		{
	 		...
	 		{ 
				latticeCoordinate[0]=(tmp_a + 1) * 0.5;
				latticeCoordinate[1]=(tmp_b + 1) * 0.5;
			}
			else
			{
				latticeCoordinate[0]=(tmp_a + tmp_b + tmp_c) * 0.3;
				latticeCoordinate[1]=(tmp_a - tmp_b + tmp_c) * 0.3;
			}
		}
	}
	...
	else if(sphere_check_flag==1)
	{
		if(eye_space_flag==1)
		{
			if(vertical_flag==1)
	  		{
				texCoord[0]=(pos[0] + 1) * 0.75;
				texCoord[1]=(pos[1] + 1) * 0.75;
			}
			else if(slant_flag==1)
	  		{
				texCoord[0]=(pos[0] + pos[1] + pos[2]) * 0.45;
				texCoord[1]=(pos[0] - pos[1] + pos[2]) * 0.45;
			}
		...
		else if(object_space_flag==1)
		{
			if(vertical_flag==1)
	  		{ 
				texCoord[0]=(tmp_a + 1) * 0.75;
				texCoord[1]=(tmp_b + 1) * 0.75;
			}
			else if(slant_flag==1)
	  		{ 
				texCoord[0]=(tmp_a + tmp_b + tmp_c) * 0.45;
				texCoord[1]=(tmp_a - tmp_b + tmp_c) * 0.45;
			}
